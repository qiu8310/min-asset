  ...
  .options({
    ...
    image: {
      ...
      desc: 'image 配置项'
        ...
        + l('bitDepthReduction', 'true') + '  PNG: Apply bit depth reduction'
        + l('colorTypeReduction', 'true') + ' PNG: Apply color type reduction'
        + l('paletteReduction', 'true') + '   PNG: Apply palette reduction'

        + l('progressive', 'true') + '    JPEG: Lossless conversion to progressive'
        + l('arithmetic', 'false') + '    JPEG: Use arithmetic coding http://en.wikipedia.org/wiki/Arithmetic_coding'

        + l('interlaced', 'false') + '    GIF: Interlace gif for progressive rendering'
        + l('optimizationLevel', '2') + ' GIF: Select an optimization level between 1 and 3'
        + l('colors', '0') + '            GIF: Reduce the number of distinct colors. Num must be between 2 and 256'
...
function minCallback(file, memo, next) {
  return function(err, data) {
    ...
    if (diff < 10) {
      ...
      var r = (diff / data.originalSize * 100).toFixed() + '%'
...
async.reduce(
  ...
  function(memo, file, next) {
    ...
    fs.readFile(file, function(err, content) {
      ...
      ma(content, file, opts, minCallback(file, memo, next))
    })
  },
